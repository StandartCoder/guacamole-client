...
/**
 * RDPCAM (Remote Desktop Protocol Camera) service for Guacamole.
 * This service provides camera redirection functionality for RDP connections.
 */
angular.module('client').factory('guacWebcam', ['$injector', function guacWebcam($injector) {
     ...
     * @type String
     */
    const RDPCAM_MIMETYPE = 'application/rdpcam+h264';

    /**
     * Default camera constraints for RDPCAM.
     ...
     * legacy behavior that targeted the most recently focused client.
     */
    ...
    function saveEnabledDevicesToPreferences() {
        ...
        preferenceService.preferences.rdpcamEnabledDevices = enabledDevices;
    ...
    /**
     * Sends updated capabilities to the server via rdpcam-capabilities-update argv stream.
    ...
    function updateCapabilities(client) {
        ...
        if (enabledCameras.length === 0) {
            try {
                var stream = client.createArgumentValueStream('text/plain', 'rdpcam-capabilities-update');
     ...
     *
     * @param {!Guacamole.Client} client
     *     The client instance handling the RDPCAM stream.
     *
    ...
    function sendCapabilities(client, devices) {
        ...
        try {
            ...
            var stream = client.createArgumentValueStream('text/plain', 'rdpcam-capabilities');
            var writer = new Guacamole.StringWriter(stream);
            writer.sendText(payload);
            writer.sendEnd();
        }
        ...
        navigator.mediaDevices.enumerateDevices()
            .then(function(devices) {
                ...
                Promise.all(devicePromises).then(function(deviceCapabilities) {
                    ...
                    var savedEnabledDevices = preferenceService.preferences.rdpcamEnabledDevices;
    ...
    function startCamera(client, constraints, onState) {
        ...
        return new Promise((resolve, reject) => {
            ...
            try {
                ...
                const realStream = client.createVideoStream(RDPCAM_MIMETYPE);
                ...
                const recorder = Guacamole.CameraRecorder.getInstance(stream, RDPCAM_MIMETYPE);
    ...
    function isSupported() {
        return Guacamole.CameraRecorder.isSupportedType(RDPCAM_MIMETYPE);
    }
    ...
    function getVideoDelay() {
        var delay = preferenceService.preferences.rdpcamVideoDelay;
    ...
    function setVideoDelay(delayMs) {
        ...
        preferenceService.preferences.rdpcamVideoDelay = videoDelayMs;
        preferenceService.save();
