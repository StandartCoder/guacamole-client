...
angular.module('client').controller('clientController', ['$scope', '$routeParams', '$injector',
    ...
    const guacWebcam             = $injector.get('guacWebcam');
    ...
    $scope.cameraSupported = guacWebcam.isSupported();

    /**
     * Video delay setting (0-1000ms) bound to slider in UI.
     * @type {number}
     */
    $scope.videoDelay = guacWebcam.getVideoDelay();
    ...
    /**
     * Registers callback to update camera device list from guacWebcam service.
     */
    guacWebcam.registerCameraListCallback(function(cameras) {
    ...
    $scope.onCameraToggle = function(deviceId, enabled) {
        guacWebcam.toggleCamera(deviceId, enabled);
    };
    ...
    $scope.$on('guacClientFocused', function focusedClientChanged(event, newFocusedClient) {
        ...
        if (newFocusedClient && newFocusedClient.client) {
            ...
            guacWebcam.registerStateCallback(newFocusedClient.client, function(state) {
                $scope.$apply(function() {
        ...
        if (newFocusedClient && newFocusedClient.client && guacWebcam.isSupported()) {
    ...
    $scope.prepareCameraCapabilities = function prepareCameraCapabilities() {
        if (!$scope.focusedClient || !guacWebcam.isSupported()) {
            return;
        }

        guacWebcam.prefetchCapabilities($scope.focusedClient.client);
    };
    ...
    $scope.stopCamera = function stopCamera() {
        if ($scope.focusedClient) {
            guacWebcam.stopCamera($scope.focusedClient.client);
        }
        $scope.cameraActive = false;
    };
    ...
    $scope.updateVideoDelay = function updateVideoDelay() {
        ...
        guacWebcam.setVideoDelay(delay);
    ...
    $scope.$on('$destroy', function clientViewDestroyed() {
        ...
        guacWebcam.resetDeviceChangeHandler();
