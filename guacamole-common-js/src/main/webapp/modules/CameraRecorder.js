 ...
 *     must be "application/rdpcam+h264".
 */
Guacamole.H264CameraRecorder = function H264CameraRecorder(stream, mimetype) {
     ...
     * @type {number}
     */
    var forceIdrIntervalMs = (typeof window !== 'undefined' && window.GUAC_RDPCAM_FORCE_IDR_MS) ?
            (parseInt(window.GUAC_RDPCAM_FORCE_IDR_MS, 10) || 2000) : 2000;
    ...
    /**
     * Builds the RDPCAM frame header.
     *
     * @private
     * @param {Object} params
    ...
    var streamReceived = function streamReceived(stream) {
        ...
        // Matches FreeRDP's bitrate recommendations for RDPCAM
...
Guacamole.H264CameraRecorder.isSupportedType = function isSupportedType(mimetype) {
    ...
    return mimetype === 'application/rdpcam+h264';
...
Guacamole.H264CameraRecorder.getSupportedTypes = function getSupportedTypes() {
    ...
    return ['application/rdpcam+h264'];
